...
export function ChatBox({ serverId, userId, initialMessages }: ChatBoxProps) {
  ...
  useEffect(() => {
    ...
    socketInstance.on("message-updated", (msg: Message) => {
      ...
      (data: { messageId: string; mode: string; updatedMessage?: Message }) => {
        setMessages((prev) => {
          if (data.mode === "everyone" && data.updatedMessage) {
            return prev.map(m => m.id === data.messageId ? data.updatedMessage! : m);
          }
  ...
  const handleSendMessage = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Socket will handle the update for everyone
        socket?.emit("send-message", { serverId, message: sentMessage });
  ...
  const handleEditMessage = async (messageId: string) => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`/api/servers/${serverId}/chat/${messageId}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
      ...
      if (response.ok) {
        ...
        socket?.emit("edit-message", { serverId, message: updated });
        setEditingId(null);
        setEditText("");
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (mode === "everyone") {
          ...
          socket?.emit("delete-message", { serverId, messageId, mode, updatedMessage });
        } else {
        ...
        setActiveMenuId(null);
        toast.success(mode === "everyone" ? "Pesan dihapus untuk semua" : "Pesan dihapus");
      }
    } catch (error) {
    ...
    <div className="flex flex-col h-full bg-zinc-50 dark:bg-zinc-950">
      ...
      >
        {messages.map((msg, i) => {
          ...
          const showName = !isMe && !isDeleted && (!prevMsg || prevMsg.userId !== msg.userId);
            ...
            >
              ...
              {!isDeleted && !isEditing && (
                <button 
                  ...
                  className={cn(
                    ...
                    activeMenuId === msg.id && "opacity-100 bg-zinc-100 dark:bg-zinc-700"
                  )}
                >
                  <MoreVertical size={14} className="text-zinc-400 dark:text-zinc-500" />
                </button>
              )}
              ...
              <div className={cn("flex flex-col", isMe ? "items-end" : "items-start")}>
                ...
                {activeMenuId === msg.id && (
                  ...
                  <button onClick={() => { setReplyingTo(msg); setActiveMenuId(null); }} className="w-full flex items-center gap-3 px-3 py-2 text-sm text-zinc-600 dark:text-zinc-400 hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-colors">
                    <Reply size={14} /> Balas
                  </button>
                  <button onClick={() => copyMessage(msg.message)} className="w-full flex items-center gap-3 px-3 py-2 text-sm text-zinc-600 dark:text-zinc-400 hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-colors">
                    <Copy size={14} /> Salin
                  </button>
                  {isMe && (
                    <>
                      <button onClick={() => { setEditingId(msg.id); setEditText(msg.message); setActiveMenuId(null); }} className="w-full flex items-center gap-3 px-3 py-2 text-sm text-zinc-600 dark:text-zinc-400 hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-colors">
                        <Pencil size={14} /> Edit
                      </button>
                      <button onClick={() => { setShowDeleteConfirm(msg); setActiveMenuId(null); }} className="w-full flex items-center gap-3 px-3 py-2 text-sm text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-950 transition-colors">
              ...
              <div className={cn(
                ...
                isDeleted && "italic opacity-60 bg-zinc-200 dark:bg-zinc-800 text-zinc-500 dark:text-zinc-500 border-none"
              )}>
                {/* Replied Message Snippet */}
                {msg.replyTo && !isDeleted && (
                  <div className={cn(
                    "mb-2 p-2 rounded-lg border-l-4 bg-black/5 dark:bg-white/5 text-xs truncate max-w-full",
                    isMe ? "border-blue-300" : "border-zinc-300 dark:border-zinc-600"
                  )}>
                    <p className="font-semibold opacity-70 mb-0.5">{msg.replyTo.user.fullName}</p>
                ...
                {isEditing ? (
                  <div className="flex flex-col gap-2 min-w-[200px]">
                    ...
                    <div className="flex justify-end gap-2">
                      <button onClick={() => setEditingId(null)} className="text-[10px] uppercase font-bold opacity-70 hover:opacity-100">Batal (Esc)</button>
                      <button onClick={() => handleEditMessage(msg.id)} className="text-[10px] uppercase font-bold flex items-center gap-1">Simpan (Enter)</button>
                    </div>
                  </div>
                ) : (
                  <div className="flex flex-col">
                    {showName && <span className="text-[11px] font-semibold text-blue-600 dark:text-blue-400 mb-0.5">{msg.user.fullName}</span>}
                    <p className="whitespace-pre-wrap break-words leading-relaxed">{msg.message}</p>
                    <div className={cn(
                      "flex items-center gap-1 self-end mt-1 text-[10px] opacity-60",
                      isMe ? "text-blue-100" : "text-zinc-400 dark:text-zinc-500"
                    )}>
      ...
      {showDeleteConfirm && (
        <div className="fixed inset-0 bg-black/50 dark:bg-black/70 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-in fade-in duration-200">
          <div className="bg-white dark:bg-zinc-900 rounded-2xl p-6 max-w-sm w-full shadow-xl space-y-4">
            <h3 className="text-lg font-semibold text-zinc-900 dark:text-zinc-100">Hapus pesan?</h3>
            <div className="space-y-1">
              <Button 
                onClick={() => handleDeleteMessage(showDeleteConfirm.id, "everyone")}
              ...
              <Button 
                onClick={() => handleDeleteMessage(showDeleteConfirm.id, "me")}
        ...
        <div className="max-w-4xl mx-auto flex flex-col gap-2">
          {replyingTo && (
            <div className="flex items-center justify-between bg-zinc-50 dark:bg-zinc-900 px-3 py-2 rounded-xl border-l-4 border-blue-500 animate-in slide-in-from-bottom-2 duration-200">
              <div className="min-w-0">
                <p className="text-xs font-semibold text-blue-600 dark:text-blue-400">Membalas {replyingTo.user.fullName}</p>
                <p className="text-xs text-zinc-500 dark:text-zinc-400 truncate">{replyingTo.message}</p>
              </div>
              <button onClick={() => setReplyingTo(null)} className="p-1 text-zinc-400 dark:text-zinc-500 hover:text-zinc-600 dark:hover:text-zinc-300 shrink-0">
                <X size={16} />
              </button>
            </div>
          )}
          ...
          <form onSubmit={handleSendMessage} className="relative">
            <div className="flex items-end gap-2 bg-zinc-100 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-2xl px-4 py-2 focus-within:border-blue-300 dark:focus-within:border-blue-700 focus-within:ring-2 focus-within:ring-blue-50 dark:focus-within:ring-blue-950 transition-all">
              ...
              >
                {loading ? <Loader2 className="animate-spin" size={16} /> : <Send size={16} className="ml-0.5" />}
              </Button>
            </div>
          </form>
        </div>
